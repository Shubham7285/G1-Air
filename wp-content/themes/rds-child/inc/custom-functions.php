...
$get_rds_template_data_array = json_decode($rdsTemplate, true);
	...
	function cta_shortcode_shortcode($attr) {
		...
		$classes = get_body_class();
		 $width_class="w-100";
		 $btn_width_class="max_w_420";
		 $col_left_fullwidth_class="col-lg-9";
		 $col_right_fullwidth_class="col-lg-3";
	if (!in_array('elementor-page-40551', $classes)) {
		$width_class="max_w_730";
		$btn_width_class="";
		$col_left_fullwidth_class="col-lg-7";
		$col_right_fullwidth_class="col-lg-5";
	}
		$rdsTemplate = get_option("rds_template");
		$get_rds_template_data_array = json_decode($rdsTemplate, true);
		...
		if (isset($attr["icon_class"])) {
			...
			$iconClass = $get_rds_template_data_array["globals"]["in_content_cta"][
					"icon_class"
			];
		}
		if (isset($attr["heading"])) {
			...
			$heading = $get_rds_template_data_array["globals"]["in_content_cta"][
					"heading"
			];
		}
		if (isset($attr["title_class"])) {
			...
			$title_class = $get_rds_template_data_array["globals"]["in_content_cta"][
					"title_class"
			];
		}
		if (isset($attr["button_class"])) {
			...
			$button_class = $get_rds_template_data_array["globals"]["in_content_cta"][
					"button_class"
			];
		}
		if (isset($attr["button_text"])) {
			...
			$buttonText = $get_rds_template_data_array["globals"]["in_content_cta"][
					"button_text"
			];
		}
		$i = 0;
		if (isset($attr["button_link"])) {
			...
			$buttonLink = $get_rds_template_data_array["globals"]["in_content_cta"][
					"button_link"
			];
			$i++;
		}
	if (isset($attr["phone"])) {
		...
		if (isset($attr["target"])) {
			...
			$target = $get_rds_template_data_array["globals"]["in_content_cta"][
					"target"
					] == "true" ? "_blank" : "_self";
		}
		$schedule_id = "";
		...
		if (isset($attr["telephone_class"])) {
			...
			$telephone_class = $get_rds_template_data_array["globals"]["in_content_cta"][
					"telephone_class"
			];
		}
		$counteyCode = $get_rds_template_data_array["site_info"]["country_code"];
		$backgroungImage = get_stylesheet_directory_uri() .
				"/img/in-content-cta/cta-img.webp";
		$img1x = [
			get_exist_image_url("in-content-cta", "cta-img"),
			get_exist_image_url("in-content-cta", "cta-img"),
			get_exist_image_url("in-content-cta", "m-img-bg"),
		];
		$img2x = [
			get_exist_image_url("in-content-cta", "cta-img@2x"),
			get_exist_image_url("in-content-cta", "cta-img@2x"),
			get_exist_image_url("in-content-cta", "m-cta-img@2x"),
		];

		$img3x = [
			get_exist_image_url("in-content-cta", "cta-img@3x"),
			get_exist_image_url("in-content-cta", "cta-img@3x"),
			get_exist_image_url("in-content-cta", "m-cta-img@3x"),
		];
		...
		echo do_shortcode(
			'[custom-bg-srcset class="content-in-cta" img1x="' .
				$img1x .
				'" img2x="' .
				$img2x .
				'" img3x="' .
		...
		if (
			...
			echo '<div class="d-flex rpx_gap_20  rpx_gap_lg_12 rpx_gap_xl_0 content-in-cta content-cta-section color_secondary_bg flex-column flex-md-row justify-content-lg-between justify-content-md-evenly align-items-md-center w-100  rpx_radius_10">
			<div class="cta_heading '.$title_class .'"><div class="cta_title text_24 line_height_30 true_white text_medium text-uppercase rpx_mb_6  mb-0">' .
			$heading .
			'</div><span class="text_24 line_height_30 true_white text-uppercase   mb-0 text_medium"><a href="tel:'.$counteyCode . $telLink .'" class="text_24 line_height_30 cta_phone_link no_hover_underline true_white  ' .$telephone_class.'">'.$telLink .'</a></span></div>
			<div class="cta-center-border"></div>
			<div class="cta_link_title'.$button_class .' cta_button_text"><a href="/contact-us" target="'.$target .'" class="no_hover_underline true_white--imp  link_text_alt_btn link_text_btn "><span class="d-inline-block align-middle text_24 text_bold true_white line_height_24">Schedule Online</span> <i class="icon-chevron-right1 text_18 line_height_18 align-middle true_white"></i></a></div>
			</div>';
		} elseif (
				$get_rds_template_data_array["globals"]["in_content_cta"][
				"variation"
			...
			$content = $get_rds_template_data_array['globals']['in_content_cta']['content'];
			echo '<div><span  class=" d-block no_hover_underline mb-4 rounded-20 overflow-hidden ct_gap">
	   <div class=" m-0 ct_gap_inner position-relative color_secondary_bg ">
		   <div class=" order-2 order-xl-1 p-4 color_secondary_bg rounded-left content_p_30 content_p_lg_40 cta_left_col">
			   <div class=" p-0">
			   <span class="h5 true_white--imp d-block mb-3 pb-1">' .
			$heading .
			'</span>
			   <span class="p true_white--imp mb-3 d-block pb-1">'.$content.'</span>
			   <div class=" cta-inner-btn '.$btn_width_class.'">
				   <div class=" mb-xxxl-0 mb-3 pb-1 cta-btn-left">
					   <a id="" class="' . $id . ' mw-xxl-100 no_hover_underline cta_link btn-primary w-100" href="' . $href . '" target="' . $target . '">
					   ' . $buttonText . '
					   </a>
				   </div>
				   <div class=" cta-btn-right">
					   <a class="mw-xxl-100 cta_call_link cta_link btn btn-secondary w-100 no_hover_underline ' . $telephone_class . ' " href="tel:' . $counteyCode . $telLink . '">
					   ' . $telLink . '
					   </a>
				   </div>
			   </div>
...
<div class="accordion-item  rounded-0 border-1 mb-3">
                <div  class="accordion-header">
                       <h3 class="collapsed rpx_p_16 mb-0 position-relative d-flex align-items-center" data-bs-toggle="collapse" data-bs-target="#' .
		$id .
		...
		'<i class="' .
		$iconClass .
		' text_20 line_height_28 position-absolute right-15 top-18 color_primary" ></i></h3>
                </div>
                <div id="' .
		$id .
...
function bc_read_more_shortcode($attr)
{
	...
	echo '<a class="' .
		$background_color .
		' bc_toggle_btn bc_toggle_btn_closed bc_toggle_content link_text_btn d-flex gap_5" ' .
		$icon_open .
		" " .
		$icon_close .
		' href="#' .
	...
	if (isset($atts["page_id"])) {
		...
		if ($query->have_posts()):
			while ($query->have_posts()):
                ...
                $ret = explode(",", $newCity);
                ?>
                <div class="text-start hide-area service-areas">
                    <a class=" true_black--imp d-block text-start line_height_28" href="<?php the_permalink(); ?>"><?php echo $ret[0]; ?></a> 
                </div>
                <?php
			endwhile;
...
// gform_submit_button
add_action('after_setup_theme', 'remove_parent_gravityforms_button');
function remove_parent_gravityforms_button() {
    remove_filter("gform_submit_button", "rds_gravity_submit_button", 10);
}
 
add_filter("gform_submit_button", "child_theme_gravity_submit_button", 10, 2);
function child_theme_gravity_submit_button($button, $form) {
   
    return "<div class='text-center pt-2'><button class='btn btn-primary  w-lg-100 rds_gform_submit' id='gform_submit_button_{$form["id"]}'>" . $form["button"]["text"] . "<i class=' icon-circle-chevron-right2
'></i></button></div>";
}